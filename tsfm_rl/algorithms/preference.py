...
def train_preference(run: Run, tau=0.05):
    ...
    for step in range(cfg.steps):
        ...
        inputs, roles, cpm, n_ctx = env.build(actions); q_ref = run.reference(inputs, roles, cpm)["logits"][:, 0, n_ctx - 1, :env.bank.H, :]
        q, y, _ = env.forecast(pol, actions)
        with torch.no_grad():
            samples = sample_forecasts(q.detach(), cfg.K, run.g); r = torch.stack([REWARDS[cfg.reward](samples[k][..., None].expand(-1, -1, 9), y, q_ref) for k in range(cfg.K)])
            lp_ref = torch.stack([logpdf(q_ref, samples[k]).mean(-1) for k in range(cfg.K)])
        lp = torch.stack([logpdf(q, samples[k]).mean(-1) for k in range(cfg.K)])
        losses = []
        ...
        loss = torch.cat(losses).mean() + cfg.kl * ((q - q_ref) ** 2).mean()
...
def train_context(run: Run, sft=False, tau=0.05):
    cfg, env = run.cfg, run.env; frozen = run.reference
    ...
    for step in range(cfg.steps):
        ...
        with torch.no_grad():
            for j in range(P):
                q, y, _ = env.forecast(frozen, [p[j] for p in proposals]); rewards[:, j] = env.reward(q, y)
